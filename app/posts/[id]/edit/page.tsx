...
export default function EditPostPage({
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setPostError(err instanceof Error ? err.message : "Nie udało się pobrać posta");
      } finally {
        setLoading(false);
      }
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const result = await updatePost({ id: post.id, userId: post.userId, title: post.title, body: post.body });
      setUpdatedPost(result);
    } catch (err: unknown) {
      setPostError(err instanceof Error ? err.message : "Nie udało się zaktualizować posta");
    }
  }
  ...
  if (loading) return <p className="p-6 text-zinc-400">Ładowanie...</p>;
  if (postError) return <p className="p-6 text-red-400">{postError}</p>;
    ...
    <main className="p-6 max-w-xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <input id="titleInput" className="w-full p-2 border rounded" value={post.title} onChange={(e) => updateField("title", e.target.value)} required />
        </div>
        <div>
          <label htmlFor="bodyInput" className="block mb-1 font-medium">Treść</label>
          <textarea id="bodyInput" className="w-full p-2 border rounded" rows={5} value={post.body} onChange={(e) => updateField("body", e.target.value)} required />
        </div>
      ...
      <Link href="/" className="inline-block px-4 py-2 bg-zinc-800 hover:bg-zinc-700 text-zinc-100 rounded-lg transition mt-6">
        ← Powrót do listy
      </Link>
    </main>
  );
